...
def analyze_with_gemini(text, api_key):
    try:
        # Önce kullanılabilir modelleri listele
        ...
        for m in models_data.get("models", []):
            if "generateContent" in m.get("supportedGenerationMethods", []):
                name = m.get("name", "")
                if "models/" in name:
                    valid_models.append(name.split("models/")[1])
        
        # En iyi modelleri önceliklendir
        chosen_model = None
        ...
        if not chosen_model:
            return {"errors": [], "corrected_text": text, "summary": "API hatası: Hesabınızda kullanılabilecek hiçbir model bulunamadı."}
        ...
        data = {
            "contents": [{"parts": [{"text": f"{GEMINI_SYSTEM_PROMPT}\n\nAnaliz edilecek metin:\n\"{text}\""}]}],
        ...
        try:
            err_msg = json.loads(err_msg).get("error", {}).get("message", err_msg)
        except:
            pass
        return {"errors": [], "corrected_text": text, "summary": f"API hatası: {err_msg}"}
    except Exception as e:
        return {"errors": [], "corrected_text": text, "summary": f"Sistem hatası: {str(e)}"}
...
if check_clicked and text_input:
    ...
    # --- Sonuçlar ---
    if use_ai and ai_summary and "API hatası" in ai_summary:
        st.error(f"🚨 {ai_summary}")
        # API hatası varsa başarı kutusunu göstermemek için total'i değiştiriyoruz
        total = -1
